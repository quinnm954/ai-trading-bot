...
async function processUserPositions(supabase: any, userId: string, isPaperMode: boolean, availablePairs: Set<string>) {
  ...
  if (hasReachedTarget) {
    ...
    for (const position of positions) {
      ...
      // Execute REAL crypto sell
      if (!isPaperMode) {
        console.log(`💰 Executing REAL crypto sell: ${quantity} ${position.symbol}`);

        const sellResult: { success: boolean; usdValue?: number; error?: string } =
          await executeCoinbaseSell(position.symbol, quantity);
  ...
  for (const position of positions) {
    ...
    if (entryPrice <= 0) {
      ...
      if (shouldSellLegacy) {
        console.log(`💰 ${isPaperMode ? 'PAPER ' : ''}SELLING LEGACY POSITION: ${quantity} ${position.symbol} worth $${positionValue.toFixed(2)} (no entry price)`);
    ...
    const oneToOneFloorPct = Math.max(
      COINBASE_ROUND_TRIP_FEE + MIN_NET_EXIT_PCT,
      COINBASE_ROUND_TRIP_FEE + posNetLossPct
    );
    // Absolute floor: a profit-taking exit must still clear the round trip plus MIN_NET_EXIT.
    ...
    // ⏳ MAX HOLD — a scalp that resolved neither way for MAX_HOLD_MINUTES is closed at
    ...
    if (hitHardTakeProfit || hitRotationTarget || hitTrailingStop || hitStopLoss || hitMaxHold) {
      ...
      if (!didDirectConversion && !isPaperMode) {
        console.log(`💰 Selling to cash: ${quantity} ${position.symbol}`);
      ...
      const reasoningText = hitHardTakeProfit
        ...
        : hitTrailingStop 
          ...
          : hitMaxHold
            ? `⏳ Max hold reached (${positionAgeMinutes.toFixed(0)}m) — closed at ${pnlPercent.toFixed(3)}% to free the slot`
      ...
      if (hitHardTakeProfit || hitRotationTarget || hitTrailingStop || (hitMaxHold && actualPnl >= 0)) takeProfitCount++;
      else stopLossCount++;
    } else {
